 ...
 * @LastEditTime: 2025-07-22 18:24:56
...
typedef enum{
    ...
    CMD_DEV_CONFIG          = 61,           //宽带台设备参数配置指令 (42)
    CMD_CH1_CONFIG          = 63,           //带台业务通道1参数配置指令 (43)
...
/* 自检状态信息 */
// typedef struct {
//     //uint16_t header;              // 必须为0x4C4D
//     uint16_t device_id;             // 设备编号
//     uint16_t cpu_temp;               // 综合信息处理模块温度(单位:℃)
//     uint8_t  power_status;          // 供电状态     1：正常  0：异常
//     uint8_t  clock_lock;           // 综合信息处理模块时钟锁定   1：锁定 0：失锁
//     uint8_t  channel_a_lock;       // 信道A本振锁定  1：锁定 0：失锁
//     uint8_t  channel_b_lock;       // 信道B本振锁定
//     uint8_t  channel_c_lock;       // 信道C本振锁定
//     uint8_t  channel_d_lock;       // 信道D本振锁定
//     uint16_t channel_temp;         // 信道温度        
//     uint16_t amplifier_temp;       // 功放温度    
//     uint8_t  amplifier_protection; // 功放开路保护状态   1：故障 0：正常
//     uint8_t  reserved[4];          // 预留位 4B
// } kd_selfcheck_report_t;

// // 参数状态信息上报数据
// typedef struct {
//     //uint16_t header;       // 
//     uint16_t device_id;         // 设备编号 
//     uint8_t  conne_status; // 联通状态 
//     double   longitude;         // 位置信息(经度)
//     double   latitude;          // 位置信息(纬度) 
//     double   height;          // 位置信息(高度)
//     uint8_t   channel_quality[112];   // 信道质量 
//     uint8_t    snr[112];               // 信噪比 
//     uint8_t    rssi[112];       // 接收电平 
//     uint8_t    noise[112];       // 噪声值   显示为负值
//     uint8_t  time_quality;      // 时间质量 
//     uint8_t  reserved[4] ;       // 预留位 
// } kd_status_report_t;
...
typedef struct{
    ...
    uint8_t interf_cancel;       /* 干扰对消功能 */
    uint8_t decision_mode;       /* 决策模式 */
    uint8_t cluster_head;        /* 簇首标识 */
    uint8_t center_node;         /* 中心节点标识 */
...
typedef struct {
    ...
    uint8_t     mcs_mode;               //调制编码方式  1-8 
    uint8_t     work_mode;              //工作模式    0：定频 1：跳频
    uint8_t     bw;                     //带宽        0：2.5MHz、1：5MHz、2：10MHz、3：20MHz
    uint16_t    freq;                   //工作频率    225MHz-2.5GHz，每1MHz为一个步进，225MHz对应频点1，225MHz+i×1MHz为第i个频点；
    uint8_t     hop_rate;               //跳频速率    
    uint8_t     hop_pattern;            //跳频图案
    uint8_t     reserved_2;             //扩频因子
...
{
   APP_HEAD  app_head;
   //kd_selfcheck_report_t app_info;
   uint16_t  crc;
